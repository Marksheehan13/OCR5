...
def _secret(name: str) -> str:
    """Read a Streamlit secret if present, without requiring it to exist."""
    try:
...
# Local .env values are loaded by src.settings at import time.
...
with st.sidebar:
    ...
    supabase_url = st.text_input(
        "Supabase URL",
        value=saved_supabase_url,
        placeholder="https://xxxxx.supabase.co",
    )
    supabase_key = st.text_input(
        "Supabase key",
        value=saved_supabase_key,
        type="password",
    )
