...
def verify_extraction(image_path: str, result: InvoiceExtraction, api_key: str, model: str) -> InvoiceExtraction:
    ...
    try:
        ...
        response = litellm.completion(model=model, api_key=api_key, max_tokens=768, messages=[
    ...
    for name in ("date", "supplier", "amount"):
        ...
        status = data.get(f"{name}_status")
        value = data.get(name)
        ...
        if status == "corrected" and value not in (None, ""):
            ...
            corrections.append(f"{name}: {old!r} → {value!r}")
        ...
        if reason:
            field.reasons.append(f"Second-pass verifier: {reason}")
