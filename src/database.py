...
def get_invoice_analytics(client_id=None) -> dict:
    ...
    by_supplier, by_month = defaultdict(float), defaultdict(float)
    for row in invoices:
        if row[3] is None: continue
        by_supplier[row[1] or "Unknown supplier"] += float(row[3])
        if row[2]: by_month[str(row[2])[:7]] += float(row[3])
    return {"client_id": client_id, "invoice_count": len(invoices), "line_item_count": len(items), "total_spend": sum(valid_amounts), "total_vat": sum(valid_vat), "average_invoice_value": sum(valid_amounts) / len(valid_amounts) if valid_amounts else 0.0, "spend_by_supplier": dict(sorted(by_supplier.items(), key=lambda x:x[1], reverse=True)), "spend_by_month": dict(sorted(by_month.items()))}
...
def get_supplier_item_analysis(client_id=None) -> list[dict]:
    ...
    groups = {}
    for item in get_client_invoice_line_items(client_id) if client_id is not None else get_all_invoice_line_items():
...
def get_item_price_comparisons(client_id=None) -> list[dict]:
    ...
    for suppliers in items.values():
        ...
        comparisons.append({"description":suppliers[0]["description"],"suppliers":suppliers,"lowest_price":min(prices),"highest_price":max(prices),"price_difference":max(prices)-min(prices)})
...
def get_all_invoices(client_id=None):
    ...
    if client_id is not None: query=query.eq("client_id",client_id)
